...
class Manipulator
{
    private _scene: ValjangEngine.Scene;
    private _engine: ValjangEngine.Engine;
    private _camera: ValjangEngine.ArcRotateCamera;
    private _pickResult: ValjangEngine.PickingInfo;
    private _pickDeltaFromMeshOrigin: ValjangEngine.Vector3;
    private _forcedMeshSelect: ValjangEngine.Mesh;
    private _curSelectedMesh: ValjangEngine.Mesh;
    private _selectedMeshMaterial: ValjangEngine.StandardMaterial;
    private _xDirMeshMaterial: ValjangEngine.StandardMaterial;
    private _yDirMeshMaterial: ValjangEngine.StandardMaterial;
    private _zDirMeshMaterial: ValjangEngine.StandardMaterial;
    private _allDirMeshMaterial: ValjangEngine.StandardMaterial;
    private _mode: ManipulatorMode;
    private _meshScaleX: ValjangEngine.Mesh;
    private _meshScaleY: ValjangEngine.Mesh;
    private _meshScaleZ: ValjangEngine.Mesh;
    private _meshScaleAll: ValjangEngine.Mesh;
    private _initialScale: ValjangEngine.Vector3;
    private _meshRotPitch: ValjangEngine.Mesh;
    private _meshRotYaw: ValjangEngine.Mesh;
    private _meshRotRoll: ValjangEngine.Mesh;
    private _initialRot: ValjangEngine.Quaternion;

    public constructor(engine: ValjangEngine.Engine, scene: ValjangEngine.Scene, camera: ValjangEngine.ArcRotateCamera)
    {
        this._scene = scene;
        this._engine = engine;
        this._camera = camera;
    ...
    public onPointerDown(e: PointerEvent, p: ValjangEngine.PickingInfo)
    {
        let pickResult: ValjangEngine.PickingInfo = p; //this._scene.pick(e.clientX, e.clientY);
        switch(this._mode)
        {
    ...
    public onPointerMove(e: PointerEvent, p: ValjangEngine.PickingInfo)
    {
        if((this._pickResult != null) && (this._pickResult.pickedMesh != null))
        {
            switch(this._mode)
            {
                case ManipulatorMode.Move:
                    {
                        let curPickRay: ValjangEngine.Ray = this._scene.createPickingRay(e.clientX, e.clientY, null, this._camera);
                        ...
                        let projPos: ValjangEngine.Vector3 = curPickRay.origin.add(curPickRay.direction.scale(origToCurPos.length()));
                ...
                case ManipulatorMode.Rotate:
                    {
                        let curPickRay: ValjangEngine.Ray = this._scene.createPickingRay(e.clientX, e.clientY, null, this._camera);
                        let curDraggedPickPoint: ValjangEngine.Vector3 = curPickRay.origin.add(curPickRay.direction.scale(this._pickResult.distance));
                        let updateManipulatorsTransforms: boolean = false;
                        let dirMeshOriginToPickOrigin: ValjangEngine.Vector3 = this._pickResult.pickedPoint.subtract(this._curSelectedMesh.position).normalize();
                        let dirMeshOriginToCurDraggedPoint: ValjangEngine.Vector3 = curDraggedPickPoint.subtract(this._curSelectedMesh.position).normalize();
                        let dot: number = ValjangEngine.Vector3.Dot(dirMeshOriginToPickOrigin, dirMeshOriginToCurDraggedPoint);
                        if(dot < 1.0)
                        {
                            let angle: number = Math.acos(dot);
                            let cross: ValjangEngine.Vector3 = ValjangEngine.Vector3.Cross(dirMeshOriginToPickOrigin, dirMeshOriginToCurDraggedPoint);
                            if(this._pickResult.pickedMesh == this._meshRotPitch)
                            {
                                // Check if we are rotating clockwize or not and modify angle accordingly
                                let xAxis: ValjangEngine.Vector3 = this._curSelectedMesh.getWorldMatrix().getRow(0).toVector3();
                                xAxis.normalize();
                                if(ValjangEngine.Vector3.Dot(xAxis, cross) < 0.0)
                                    angle = -angle;
                                // Rotate selected mesh
                                let pitchQuat: ValjangEngine.Quaternion = ValjangEngine.Quaternion.RotationYawPitchRoll(0.0, angle, 0.0);
                            ...
                            {
                                // Check if we are rotating clockwize or not and modify angle accordingly
                                let yAxis: ValjangEngine.Vector3 = this._curSelectedMesh.getWorldMatrix().getRow(1).toVector3();
                                yAxis.normalize();
                                if(ValjangEngine.Vector3.Dot(yAxis, cross) < 0.0)
                                    angle = -angle;
                                // Rotate selected mesh
                                let yawQuat: ValjangEngine.Quaternion = ValjangEngine.Quaternion.RotationYawPitchRoll(angle, 0.0, 0.0);
                            ...
                            {
                                // Check if we are rotating clockwize or not and modify angle accordingly
                                let zAxis: ValjangEngine.Vector3 = this._curSelectedMesh.getWorldMatrix().getRow(2).toVector3();
                                zAxis.normalize();
                                if(ValjangEngine.Vector3.Dot(zAxis, cross) < 0.0)
                                    angle = -angle;
                                // Rotate selected mesh
                                let rollQuat: ValjangEngine.Quaternion = ValjangEngine.Quaternion.RotationYawPitchRoll(0.0, 0.0, angle);
                ...
                case ManipulatorMode.Scale:
                    {
                        let curPickRay: ValjangEngine.Ray = this._scene.createPickingRay(e.clientX, e.clientY, null, this._camera);
                        let curDraggedPickPoint: ValjangEngine.Vector3 = curPickRay.origin.add(curPickRay.direction.scale(this._pickResult.distance));
                        let curPickOriginToDraggedPickPoint: ValjangEngine.Vector3 = curDraggedPickPoint.subtract(this._pickResult.pickedPoint);
                        let meshBsphere: ValjangEngine.BoundingSphere = this._curSelectedMesh.getBoundingInfo().boundingSphere;
                        let realRadius: number = meshBsphere.radius / Math.sqrt(3.0);  // ValjangEngine bsphere radius is incorrect
                        if(this._pickResult.pickedMesh == this._meshScaleX)
                        {
                            // Project onto "right" dir
                            let xAxis: ValjangEngine.Vector3 = this._curSelectedMesh.getWorldMatrix().getRow(0).toVector3();
                            xAxis.normalize();
                            let dot: number = ValjangEngine.Vector3.Dot(xAxis, curPickOriginToDraggedPickPoint);
                        ...
                        else if(this._pickResult.pickedMesh == this._meshScaleY)
                        {
                            ...
                            let yAxis: ValjangEngine.Vector3 = this._curSelectedMesh.getWorldMatrix().getRow(1).toVector3();
                            yAxis.normalize();
                            let dot: number = ValjangEngine.Vector3.Dot(yAxis, curPickOriginToDraggedPickPoint);
                            let resultingAbsoluteScale: number = dot / realRadius;
                            // Scale selected mesh
                        ...
                        else if(this._pickResult.pickedMesh == this._meshScaleZ)
                        {
                            ...
                            let zAxis: ValjangEngine.Vector3 = this._curSelectedMesh.getWorldMatrix().getRow(2).toVector3();
                            zAxis.normalize();
                            let dot: number = ValjangEngine.Vector3.Dot(zAxis, curPickOriginToDraggedPickPoint);
                            let resultingAbsoluteScale: number = dot / realRadius;
                            // Scale selected mesh
                        ...
                        else if(this._pickResult.pickedMesh == this._meshScaleAll)
                        {
                            ...
                            let skewAxis: ValjangEngine.Vector3 = xAxis;
                            skewAxis.addInPlace(yAxis);
                            skewAxis.addInPlace(zAxis);
                            skewAxis.scaleInPlace(1.0 / 3.0);    // Normalize
                            let dot: number = ValjangEngine.Vector3.Dot(skewAxis, curPickOriginToDraggedPickPoint);
                            let resultingAbsoluteScale: number = dot / realRadius;
                            // Scale selected mesh
                            this._curSelectedMesh.scaling.x = Math.max(0.001, this._initialScale.x + resultingAbsoluteScale);
                            this._curSelectedMesh.scaling.y = Math.max(0.001, this._initialScale.y + resultingAbsoluteScale);
    ...
    public onPointerUp(e: PointerEvent, p: ValjangEngine.PickingInfo)
    {
        switch(this._mode)
        {
            case ManipulatorMode.Move:
    ...
    public ForceMeshSelection(mesh: ValjangEngine.Mesh)
    {
    ...
    private SelectObject(mesh: ValjangEngine.Mesh)
    {
        mesh.material = this._selectedMeshMaterial;
        this._curSelectedMesh = mesh;
        // Create manipulator meshes
        let meshBbox: ValjangEngine.BoundingBox = this._curSelectedMesh.getBoundingInfo().boundingBox;
        ...
        let pivotTranslate: ValjangEngine.Matrix = ValjangEngine.Matrix.Translation(0, -meshBbox.extendSize.x / 2.0, 0);
        let pivotRot: ValjangEngine.Matrix = ValjangEngine.Matrix.RotationZ(Math.PI / 2.0);
        ...
        let manipulatorOffset: ValjangEngine.Vector3 = this._meshScaleAll.getBoundingInfo().boundingBox.extendSize.scale(-0.5);
